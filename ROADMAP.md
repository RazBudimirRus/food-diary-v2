...
**Открытые баги:** PERF-01 (медленный КБЖУ, высокий) · BUG-10 (фото с мобилки, высокий).
**Закрыты 2026-08-07:** BUG-08 (ClamAV удалён, Вариант Б) · BUG-11 (миграция 0009 не была в `_journal.json`). UX-S3-\* закрыты в v2.24.4.
...
1. **PERF-01** — ускорить расчёт КБЖУ (сейчас 5–15 с из-за thinking mode у `deepseek-v4-flash`). UX-блокер.
2. **BUG-10** — загрузка фото с мобильного телефона.
...
- Варианты решения:
  1. Параметр `thinking: { budget_tokens: 0 }` для отключения thinking у v4-flash — наименее инвазивно, пробовать первым
