...
> **Актуальный бэклог** — в разделе [«Таблица приоритетов фаз»](#таблица-приоритетов-фаз) внизу файла.
> Всё остальное — исторические описания фаз; сверяйтесь со статусом в шапке каждого раздела.
...
| v2.24.5 · 2026-07-25 | Фикс загрузки фото | ✅ Реализовано | Загрузка фото не работала из-за CSRF — исправлен `lib/queryClient.ts` |
| v2.24.6 · 2026-07-25 | Фикс предпросмотра фото | ✅ Реализовано | 401 на `img src` — отдача фото через `routes/photos.ts` вместо прямого URL |
| v2.24.7 · 2026-07-25 | Фикс фото после истечения токена | ✅ Реализовано | Новый хук `hooks/use-photo-url.ts` — фото не пропадают после истечения access token |
| v2.24.8 · 2026-07-25 | Удаление фото из карточки | ✅ Реализовано | Кнопка удаления фото в MealCard |
| v2.25.1 · 2026-07-28 | DeepSeek thinking fix | ✅ Реализовано | content:null при thinking mode |
...
| v2.27.0 · 2026-08-05 | Phase 29 W0–W5 | ✅ В main | User/Doctor repos; ApiError; UI splits; bootstrap DDL removed; createApp() + boot tests |
...
> **Статус:** ✅ Реализовано (v2.20.0, превью в MealForm подтверждены в v2.23.0)
> **Приоритет:** Средний
> **Сложность:** Средняя
...
> **Статус:** ✅ Реализовано (v2.23.0) — `POST /api/meals/:id/analyze-kbju`, кнопка Sparkles в MealCard
> **Приоритет:** Высокий
> **Сложность:** Высокая
...
> **Статус:** ✅ Реализовано (v2.20.0, подтверждено в v2.23.0) — thumbnails + лайтбокс в MealCard, экспорт в excel.ts
> **Приоритет:** Высокий
> **Сложность:** Средняя
...
> **Статус:** ✅ Реализовано (v2.20.0) — флаг `kbju_manual`, migration 0007
> **Приоритет:** Высокий
> **Сложность:** Низкая
...
> **Статус:** ✅ Реализовано (v2.23.0) — `POST /api/catalog/:id/calculate-kbju`, кнопка Sparkles в CatalogPage
> **Приоритет:** Высокий
> **Сложность:** Средняя
...
> **Статус:** ✅ Реализовано (v2.11.0) — `audit_log` (migration 0004), `server/audit.ts`, `repositories/audit.ts`, таб «Журнал» в AdminPage
...
> **Сложность:** Средняя
>
> **Актуально на v2.27.0:** 303 теста в 25 файлах, все зелёные. Порог в `vitest.config.ts` — **53%** (был снижен с 55% в ходе рефакторинга Phase 29; вернуть обратно при следующей волне тестов).
...
## Фаза 36 — Интеграции с Health-платформами

> **Статус:** 📋 Запланировано
> **Приоритет:** Низкий
> **Сложность:** Высокая
> **Источник:** Аудит: «Долгосрочное — интеграции»
> **Примечание:** ранее была ошибочно пронумерована как «Фаза 35» — номер занят фазой тестирования v2 (v2.14.0).

### Подзадачи

#### 36.1 Apple Health (HealthKit)
...
> **Как читать:** статус в этой таблице обязан совпадать с полем «Статус» в соответствующем разделе выше. При закрытии задачи правьте оба места.

### Открытые задачи (актуальный бэклог)

| Фаза | Название                                      | Приоритет | Сложность | Статус                                                                         |
| ---- | --------------------------------------------- | --------- | --------- | ------------------------------------------------------------------------------ |
| 29   | Рефакторинг: расщепление монолитов            | Высокий   | Высокая   | 🚧 Частично (v2.12 skeleton → v2.26.0 → v2.27.0); остался `bot/utils/dates.py` |
...
| 36   | Интеграции с Health-платформами               | Низкий    | Высокая   | 📋 Запланировано                                                               |
...
**Открытые баги:** BUG-10 (фото с мобилки, высокий).
**Закрыты 2026-08-07:** BUG-08 (ClamAV удалён, Вариант Б) · BUG-11 (миграция 0009 не была в `_journal.json`).
**Закрыты 2026-08-09:** PERF-01 (thinking mode отключён — замерено на проде: **1826 мс** против 5–15 с, `reasoning_tokens=0`). UX-S3-\* закрыты в v2.24.4.

### Закрытые фазы

| Фаза  | Название                                            | Статус                                                                 |
| ----- | --------------------------------------------------- | ---------------------------------------------------------------------- |
| 1     | Качество кода                                       | ✅ v1.5.0                                                              |
| 2     | Безопасность и аудит                                | ✅ v1.5.0                                                              |
| 3     | Персистентность данных                              | ✅ v1.4.0                                                              |
| 4     | Административная панель                             | ✅ MVP v1.10.0, комплит v2.7.0                                         |
| 5     | Самостоятельный сброс пароля                        | ✅ v1.15.0                                                             |
| 6     | HTTPS и домен                                       | ✅ v1.4.0                                                              |
| 9     | Алертинг DeepSeek в админке                         | ✅ v1.11.0                                                             |
| 10    | Управление временем жизни сессий                    | ✅ v1.5.0                                                              |
| 11    | Аналитика и графики истории питания                 | ✅ v1.16.0                                                             |
| 14    | Мобильная оптимизация веб-приложения                | ✅ v2.5.0 — BottomNav, PWA, TodayWidget, OnboardingTour, dark mode     |
| 15    | Кабинет врача                                       | ✅ v2.9.0                                                              |
| 16    | 152-ФЗ Compliance                                   | ✅ v2.8.0 — `pd_consent_at`, PrivacyPage                               |
| 17    | Анкета пользователя (User Profile)                  | ✅ v2.8.0 — ProfileQuestionnaire                                       |
| 18    | Таргет КБЖУ от врача                                | ✅ v2.9.0                                                              |
| 20    | Профиль питания и ограничения                       | ✅ v2.9.0                                                              |
| 21    | Расширенные отчёты Excel                            | ✅ v2.22.0 — `GET /api/report/week` + `/month`                         |
| 23    | Фото питания в S3 (VK Object Storage)               | ✅ v2.9.0                                                              |
| 24    | Аудит-лог действий (Admin & Doctor)                 | ✅ v2.11.0                                                             |
| 26    | Tech Debt Sprint: срочные исправления               | ✅ v2.10.0 / v2.20.0                                                   |
| 27    | Наблюдаемость (pino, Sentry, /health, metrics)      | ✅ v2.10.0 / v2.20.0                                                   |
| 28    | Безопасность: CSRF, MFA, EXIF, ClamAV               | ✅ v2.11.0 + v2.16.0 — CSRF+EXIF+reset-pw+scrypt, MFA TOTP, ClamAV     |
| 30    | Тестирование: расширение покрытия                   | ✅ v2.13.0 — 99 тестов, coverage 40.1%                                 |
| 31    | UX-полировка: undo, skeleton, a11y, dark            | ✅ v2.15.0 — soft-delete + undo, skeletons, zod-валидация, aria-labels |
| 34    | API-документация: OpenAPI, ADR, README v2           | ✅ v2.23.0 — `server/openapi.ts`, `/api/docs`, ADR-001…004             |
| 35    | Тестирование: расширение покрытия v2                | ✅ v2.14.0 — 242 теста, coverage 56.8%                                 |
| UX-1  | Редактирование приёма пищи                          | ✅ v1.6.0                                                              |
| UX-2  | Логин пользователя в админке                        | ✅ v1.13.0                                                             |
| UX-3  | Перенос приёма между днями                          | ✅ v1.13.0                                                             |
| UX-4  | Явные даты подъёма и отбоя                          | ✅ v1.13.0                                                             |
| UX-5  | Календарные периоды аналитики                       | ✅ v1.13.0                                                             |
| UX-6  | День недели в карусели дат                          | ✅ v2.8.0 — `formatDateWithWeekday`                                    |
| UX-7  | Каталог еды пользователя                            | ✅ v2.17.0 — `/#/catalog`                                              |
| UX-8  | Автовысота комментария дня в отчёте                 | ✅ v2.8.0 — `excel.ts`                                                 |
| UX-9  | Футер / Legal-разделы (desktop+mobile)              | ✅ v2.8.0 — Footer, PrivacyPage, AboutPage                             |
| UX-10 | Редактирование приёма — bottom sheet (моб.)         | ✅ v2.17.0                                                             |
| UX-11 | Добавление позиции из дневника в каталог            | ✅ v2.17.0                                                             |
| UX-12 | «Рассчитать КБЖУ по всему дню»                      | ✅ v2.17.0                                                             |
| UX-13 | Версия приложения в футере / «О приложении»         | ✅ v2.17.0                                                             |
| UX-14 | Дата нового приёма = выбранный день                 | ✅ v2.18.0                                                             |
| UX-15 | Форма добавления приёма вверху страницы             | ✅ v2.18.0                                                             |
| UX-16 | Фото в форме создания приёма                        | ✅ v2.18.0                                                             |
| UX-17 | Мультифото: до 5 фото на приём + компактные превью  | ✅ v2.20.0 / v2.23.0 — таблица `meal_photos`                           |
| UX-18 | AI Vision: расчёт КБЖУ по фото                      | ✅ v2.23.0 — `POST /api/meals/:id/analyze-kbju`                        |
| UX-19 | Фото на карточках приёма + миниатюры/ссылки в Excel | ✅ v2.20.0 / v2.23.0                                                   |
| UX-20 | Ручной приоритет КБЖУ в анкете (не перезаписывать)  | ✅ v2.20.0 — `kbju_manual`                                             |
| UX-21 | Расчёт КБЖУ для позиций каталога через AI           | ✅ v2.23.0 — `POST /api/catalog/:id/calculate-kbju`                    |
| UX-22 | PDF-экспорт аналитики                               | ✅ v2.22.0, графики в v2.23.0                                          |
...
**Следующий шаг после v2.27.0:**

1. **BUG-10** — загрузка фото с мобильного телефона. Единственный оставшийся баг высокого приоритета.
2. **Фаза 29 (хвост)** — единая MSK-логика в `bot/utils/dates.py`.
3. Далее по продукту: Фаза 19 (AI-советник) → Фаза 25 (GigaChat как второй анализатор) → Фаза 22 (FatSecret) → Фаза 32 (лендинг, digest).
4. Инфраструктура по мере надобности: Фаза 33 (off-site backup, k6, Postgres) → Фаза 7 (WAF).
5. Мобильные приложения (Фазы 12/13) — финальный этап.
...
## BUG-11 — Миграция 0009 не зарегистрирована в `_journal.json`

> **Статус:** ✅ Исправлено (2026-08-07)
> **Приоритет:** Средний (скрытый архитектурный долг, не аварийный)
> **Версия обнаружения:** v2.27.0 (файл появился ещё в v2.24.0, `b6347ec`)

### Как обнаружено

Диагностика прода показала «10 файлов `.sql` в образе против 9 записей в `__drizzle_migrations`». Выглядело как повторение BUG-03, но это была ложная тревога.

### Причина

Drizzle-migrator читает **не каталог `migrations/`, а `migrations/meta/_journal.json`**. В журнале было 9 записей (`0000`–`0008`), а `0009_client_error_log.sql` не был зарегистрирован вообще. Поэтому 9 записей в БД **точно соответствовали журналу** — рассинхрона не было, drizzle просто никогда не знал про `0009`.

Следствие: таблица `client_errors` на проде существовала **только благодаря guarded DDL** в `server/migrate.ts`. Это нарушало правило «схема создаётся только миграциями», и TODO из BUG-03 («свернуть guarded DDL») тихо сломал бы создание таблицы на новых окружениях.

### Исправление

1. Добавлена запись `idx: 9`, `tag: "0009_client_error_log"`, `when: 1784979838000` (дата коммита `b6347ec`) в `_journal.json`
2. В сам `.sql` добавлены разделители `--> statement-breakpoint`. Без них `cold-start-check` падал с `RangeError: The supplied SQL string contains more than one statement` — при `breakpoints: true` drizzle отдаёт файл в `prepare()` одной строкой, а в миграции три оператора

Изменение хэша файла безопасно: миграция не была записана ни в журнале, ни в БД.

### Почему деплой на прод безопасен

`when` = 1784979838000 больше текущего `max(created_at)` = 1783267800000 (хэш `0008` из BUG-03), поэтому drizzle применит миграцию и запишет её хэш. SQL идемпотентен (`CREATE TABLE IF NOT EXISTS` + `CREATE INDEX IF NOT EXISTS`) — на существующей БД выполнится впустую. Restart loop, в отличие от BUG-03, невозможен: там ломались голые `ALTER TABLE ADD COLUMN`.

### Вывод для диагностики

Сравнивать `COUNT(*)` из `__drizzle_migrations` нужно с числом записей в `_journal.json`, **а не с числом файлов `.sql`**. Проверка «файлов vs записей» даёт ложные срабатывания.
...
**Статус:** ✅ Исправлено (v2.23.0) — `Dockerfile.api` переведён на `node:22-slim` (builder + runtime), добавлены Cairo/Pango для chartjs-node-canvas.
...
**Статус:** ✅ Закрыто (2026-08-07) — выбран **Вариант Б**: сервис удалён из `docker-compose.prod.yml`
**Приоритет:** Низкий
...
**Вариант Б — убрать ClamAV полностью** ← **выбран 2026-08-07**
...
Диагностика на проде показала, что ситуация хуже описанной выше: контейнер висел `Up 8 days (unhealthy)`, а сокета `clamd.sock` не было вовсе (`ENOENT`) — clamd не поднимался без баз. То есть сканирование не выполнялось уже неделями (fail-open), но каждая проверка `/api/health` писала ошибку в лог — **~2900 строк в сутки**.

- Из `docker-compose.prod.yml` удалены: сервис `clamav`, volumes `clamav_db` / `clamav_sock`, монтирование сокета и `CLAMAV_SOCKET` у `api`, а также блок `depends_on`
- **`server/s3.ts` не тронут намеренно:** `scanForViruses()` уже пропускает скан при пустом `CLAMAV_SOCKET`. Благодаря этому возврат антивируса — правка одного файла, без изменений кода
- Заодно удалён обсолетный ключ `version: "3.9"` (WARN в каждой команде `docker compose`) и устаревший IP в шапке файла

**Компенсирующая мера:** `uploadPhoto()` перекодирует любое изображение через sharp в WebP — это уничтожает вложенные в метаданные payload'ы, что закрывает основной вектор для загружаемых картинок.

**Если возвращать антивирус:** восстановить сервис и volume из истории git и вернуть `CLAMAV_SOCKET`. Перед этим решить проблему зеркала freshclam (Вариант А), иначе повторится то же состояние.

---

## BUG-09: Из каталога переносится только название, без КБЖУ

**Статус:** ✅ Исправлено (v2.24.3) — см. раздел «v2.24.3» ниже
**Приоритет:** Высокий
...
**Статус:** ✅ Исправлено (v2.28.1, 2026-08-09) — выбран **вариант 1**: `thinking: { type: "disabled" }`

> **Не путать с предыдущими фиксами DeepSeek.** v2.24.1, v2.24.2, v2.25.0 и v2.25.1 правили **корректность**
> (парсинг JSON из thinking-вывода, `content: null`, strip `<think>`), а не скорость. Thinking-блок по-прежнему
> генерируется, задержка осталась. Все эти фиксы сделаны до заведения PERF-01.
...
- Текущее состояние кода (`server/deepseek.ts`): `model: "deepseek-v4-flash"`, `max_tokens: 4000` (общий бюджет **включая** thinking-токены), `max_completion_tokens: 512`. Отключения thinking нет — большой thinking-бюджет и есть прямая причина задержки
- Варианты решения:
  1. Параметр для отключения thinking у v4-flash — наименее инвазивно, пробовать первым ← **выбран**
  2. Снизить `max_tokens` до ~800 — ограничит длину thinking-блока
  3. Переключить на `deepseek-chat` (без thinking mode) — быстрее, но менее точный
  4. Попробовать `deepseek-v3` — нет thinking mode, высокая точность
  5. Streaming endpoint — показывать индикатор прогресса пока думает (маскирует, но не решает)
- Приоритет: высокий (UX блокер)

### Что сделано (v2.28.1, 2026-08-09)

**Уточнение по API.** Гипотеза в пункте 1 указывала параметр `thinking: { budget_tokens: 0 }` — такого поля у DeepSeek нет. По актуальной документации переключатель выглядит так:

| Параметр                               | Назначение                                                                   |
| -------------------------------------- | ---------------------------------------------------------------------------- |
| `thinking: { type: "disabled" }`       | Полностью выключает thinking. У `deepseek-v4-flash` **включён по умолчанию** |
| `reasoning_effort: low \| high \| max` | Регулирует объём размышлений, только при включённом thinking                 |

Важная деталь из документации: **в thinking-режиме параметры `temperature` и `top_p` молча игнорируются** — ошибки нет, но и эффекта нет. То есть `temperature: 0.1` в коде всё это время не работал, и после отключения thinking он начал действовать. Для оценки калорий это плюс: ответы стали устойчивее между одинаковыми запросами.

**Изменения в `server/deepseek.ts`:**

- Добавлено `thinking: { type: DEEPSEEK_THINKING }`, по умолчанию `disabled`
- Добавлено `response_format: { type: "json_object" }` — JSON-mode гарантирует разбираемый объект, поэтому аварийные ветки парсинга (greedy/non-greedy regex, strip markdown-фенсов) остаются как страховка, но в норме не задействуются
- Модель вынесена в `DEEPSEEK_MODEL`, режим — в `DEEPSEEK_THINKING` (env-переключатели, откат без пересборки)
- **Бюджеты токенов сознательно не снижены** (вариант 2 отклонён). Задержку создают реально сгенерированные токены, а не величина лимита. Зато если `thinking` когда-нибудь перестанет применяться (прокси, изменение API), жёсткий лимит привёл бы к тому, что reasoning съест весь бюджет и вернёт `content: null` — ровно та авария, которую починили в v2.25.1. Высокий потолок здесь ничего не стоит и работает как страховка
- Добавлен лог `[deepseek] analyze ok in Nms (thinking=…, reasoning_tokens=…, out=…)`

**Как проверить на проде.** `reasoning_tokens` должен быть **0** — это прямое доказательство, что thinking отключён. Если там ненулевое значение, флаг перестал применяться и задержка вернулась:

```bash
sudo docker compose -f docker-compose.prod.yml logs --since 1h api | grep 'analyze ok'
```

**Покрытие.** Все четыре точки расчёта идут через одну `analyzeNutrition()`, поэтому правка покрывает их разом: `POST /api/meals/analyze`, `POST /api/meals/:id/analyze-kbju` (UX-18), `POST /api/catalog/:id/calculate-kbju` (UX-21) и `GET /api/admin/deepseek-check`. Диагностический `POST /api/admin/deepseek-raw-test` приведён к тем же параметрам и теперь отдаёт `durationMs` — иначе он показывал бы поведение, которого в бою больше нет. Добавлено 6 тестов, фиксирующих параметры запроса.

**Результат замерен на проде (деплой 2026-08-09, v2.28.1):**

```
[deepseek] analyze ok in 1826ms (thinking=disabled, reasoning_tokens=0, out=159)
```

- **1826 мс против 5–15 с** — ускорение примерно в 3–8 раз, ожидание перестало быть UX-блокером
- `reasoning_tokens=0` — API действительно применил флаг, а не просто принял его молча. Это контрольное значение: если оно станет ненулевым, задержка вернулась
- `out=159` — ответ укладывается в `max_completion_tokens: 512` с большим запасом, риска обрезки нет

**Побочная экономия.** Reasoning-токены тарифицировались как output (`$1.10`/M против `$0.27`/M за input) и учитывались в `completion_tokens`, то есть попадали в `costEstimate` таблицы `api_usage`. После отключения thinking стоимость одного расчёта КБЖУ должна заметно упасть — сравнить можно в админском дашборде DeepSeek по датам до и после 2026-08-09.

### BUG-10: Ошибка загрузки фото с мобильного телефона

**Статус:** 📋 Открыто (заведено 2026-08-03, коммит `ab7f857`)

> **Не путать с фиксами фото v2.24.5–v2.24.8** (все от 2026-07-25): они закрыли CSRF при загрузке, 401 на `img src`,
> пропадание фото после истечения access token и добавили удаление фото. Это были проблемы отдачи/клиента,
> проявлявшиеся и на десктопе. BUG-10 — именно **загрузка с мобильного**, заведён позже и остаётся открытым.

- Воспроизводится на мобильном браузере, с ПК не проверялось

**Ведущая гипотеза (HEIC + sharp), требует проверки:**

- `server/routes/upload.ts` фильтрует файлы только по `file.mimetype.startsWith("image/")` — HEIC/HEIF от iPhone проходит фильтр
- `uploadPhoto()` в `server/s3.ts` затем безусловно гонит буфер через `sharp(buffer).webp()`, причём переданный MIME игнорируется (параметр объявлен как `_mimeType`)
- sharp по умолчанию собран **без** libheif — на HEIC он бросит исключение, что даёт 500 на загрузке именно с iPhone

**План проверки:**

1. Повторить загрузку с ПК тем же файлом (JPEG) — отсечь общие проблемы multipart/CORS
2. Снять точную ошибку из `docker compose logs api` в момент загрузки с телефона
3. Если подтвердится HEIC: либо добавить явный whitelist MIME с понятной ошибкой для пользователя, либо включить поддержку HEIC (sharp с libheif) и конвертировать в WebP
4. Проверить лимит размера — современные телефоны дают 5–12 МБ на кадр (`PHOTO_MAX_*` в `server/config.ts`)
