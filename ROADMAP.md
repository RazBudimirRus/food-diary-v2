...
**Закрыты 2026-08-09:** PERF-01 (thinking mode отключён; задержку нужно подтвердить логом на проде). UX-S3-\* закрыты в v2.24.4.
...
1. **BUG-10** — загрузка фото с мобильного телефона.
2. **PERF-01 (хвост)** — подтвердить задержку логом `analyze ok` после деплоя v2.28.1.
3. **Фаза 29 (хвост)** — единая MSK-логика в `bot/utils/dates.py`.
4. Далее по продукту: Фаза 19 (AI-советник) → Фаза 25 (GigaChat как второй анализатор) → Фаза 22 (FatSecret) → Фаза 32 (лендинг, digest).
5. Инфраструктура по мере надобности: Фаза 33 (off-site backup, k6, Postgres) → Фаза 7 (WAF).
6. Мобильные приложения (Фазы 12/13) — финальный этап.
...
**Осталось измерить.** Фактическая задержка после изменения на реальном API не замерена — локально нет ключа, а платные вызовы без спроса делать не стал. Ожидание: единицы секунд вместо 5–15 с. Цифру нужно снять из лога после деплоя.
