 ...
 */
import { Utensils, BarChart3, Shield } from "lucide-react";

interface BottomNavProps {
  isAdmin: boolean;
  currentPath: "/" | "/analytics" | "/admin" | string;
}
...
function NavItem({ icon, label, href, active }: NavItemProps) {
  return (
    ...
    >
      <span className={`h-6 w-6 flex items-center justify-center rounded-lg transition-colors
...
export function BottomNav({ isAdmin, currentPath }: BottomNavProps) {
  return (
    <nav
